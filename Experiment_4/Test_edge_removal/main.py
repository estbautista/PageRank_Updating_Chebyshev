...
# Build matrices
evol_edgelist = []
for k in range(1100): evol_edgelist.append( snapshots_dict[k] )
...
for snapshot_n in range(1, 1001):
	snapshot = 1100 - snapshot_n
	
	# Update the edgelist
	evol_edgelist = []
	for k in range(snapshot): evol_edgelist.append( snapshots_dict[k] )
